...
The **Halix Platform Elements SDK** (`halix-platform-elements-sdk-test-unscoped`) provides the public TypeScript types used by Halix custom elements when interacting with Halix-provided web components such as list views and standalone details.
...
```bash
npm install halix-platform-elements-sdk-test-unscoped
```

## Usage

```ts
import type { ListViewConfig, DetailConfig } from 'halix-platform-elements-sdk-test-unscoped';
